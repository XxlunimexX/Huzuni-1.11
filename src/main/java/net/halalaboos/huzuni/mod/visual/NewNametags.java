...
public class NewNametags extends BasicMod implements RenderManager.Renderer {
	...
	private final Toggleable invisibles = new Toggleable("Invisible", "Trace to invisible entities");
	...
	private final Mode<String> healthMode = new Mode<>("Health", "Style the health will be rendered", "None", "Hearts", "Numerical", "Percentage");
	...
	public void render(float partialTicks) {
		for (Player player : getWorld().getPlayers()) {
			...
			if (shouldRenderTag(player)) {
				setupAndRender(player, renderPosition, partialTicks);
			}
		}
	 ...
	 * @param partialTicks The delta time, or partialTicks to help with interpolation.
	 */
	private void setupAndRender(Player player, Vector3d pos, float partialTicks) {
		...
		renderPlate(player, pos, scale, width, color, text, partialTicks);
	}
	 ...
	 * @param partialTicks The delta time, or partialTicks to help with interpolation.
	 */
	private void renderPlate(Player player, Vector3d pos, double scale, int width, int color, String text, float partialTicks) {
	...
	private String getHealth(Player player) {
		...
		return healthMode.getSelected() == 2 ? " " + healthFormat + String.format("%.2f", player.getHealthData().getCurrentHealth()) : healthMode.getSelected() == 3 ? " " + healthFormat + (int) (healthPercentage * 100) + "%" : "";
	}
