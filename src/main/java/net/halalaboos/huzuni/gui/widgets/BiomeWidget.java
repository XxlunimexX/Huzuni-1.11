...
public class BiomeWidget extends BackgroundWidget {
	...
	public void renderMenu(int x, int y, int width, int height) {
        ...
        Chunk currentChunk = mc.world.getChunkFromBlockCoords(new BlockPos(MathUtils.floor(mc.player.posX), ((int) mc.player.posY), MathUtils.floor(mc.player.posZ)));
        String biome = "Biome: " + currentChunk.getBiome(new BlockPos(MathUtils.floor(mc.player.posX) & 15, mc.player.posY, MathUtils.floor(mc.player.posZ) & 15), mc.world.getBiomeProvider()).getBiomeName();
